CLASS zcl_mm_shpurchaseorder DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MM_SHPURCHASEORDER IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_list TYPE STANDARD TABLE OF zmmr_shpurchaseorder.
...
##NO_HANDLER
      ...
      IF line_exists( ranges[ name = 'PURCHASEORDER' ] ).
        DATA(lrt_purchaseorder) = VALUE #( ranges[ name = 'PURCHASEORDER' ]-range ).
      ENDIF.
      ...
      SELECT purchasecontract AS PurchaseOrder
        FROM I_PurchaseContractAPI01
        WHERE ( purchasecontract IN @lrt_purchaseorder )
