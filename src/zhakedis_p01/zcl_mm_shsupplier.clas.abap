CLASS zcl_mm_shsupplier DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MM_SHSUPPLIER IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_list TYPE STANDARD TABLE OF zmmr_SHsupplier.
...
##NO_HANDLER
      ...
      IF line_exists( ranges[ name = 'SUPPLIER' ] ).
        DATA(lrt_Supplier) = VALUE #( ranges[ name = 'SUPPLIER' ]-range ).
      ENDIF.
      ...
      SELECT Supplier,
             supplierfullname
        FROM I_Supplier
        WHERE ( Supplier IN @lrt_Supplier )
              ORDER BY Supplier
