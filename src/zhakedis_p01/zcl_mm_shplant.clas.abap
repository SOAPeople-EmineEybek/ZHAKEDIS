CLASS zcl_mm_shplant DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MM_SHPLANT IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    DATA lt_list TYPE STANDARD TABLE OF zmmr_shplant.
    ...
    IF io_request->is_data_requested(  ).
      ...
      IF line_exists( ranges[ name = 'PLANT' ] ).
        DATA(lrt_PLANT) = VALUE #( ranges[ name = 'PLANT' ]-range ).
      ENDIF.
      ...
      SELECT plant,
             plantname as plantTxt
        FROM i_plant
        WHERE ( plant IN @lrt_PLANT )
              ORDER BY Plant
